    ...
    def train_model(self, model: Any, X_train: np.ndarray, y_train: np.ndarray) -> tuple:
        ...
        try:
            mlflow_tracker.log_dataset_info(
                (train_size, X_train.shape[1], X_train.shape[2]),
                (validation_size, X_train.shape[1], X_train.shape[2]),
                y_train[:train_size],
                y_train[train_size:]
            )
        except Exception as e:
            print(f"⚠️ MLflow dataset logging warning: {e}")
        ...
        try:
            mlflow_tracker.log_training_metrics(history, training_time)
        except Exception as e:
            print(f"⚠️ MLflow training metrics logging warning: {e}")
